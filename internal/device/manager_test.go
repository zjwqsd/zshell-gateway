...
func TestCapabilitiesNormalizeAndLegacyFallback(t *testing.T) {
	manager := NewManager()
	modern, ok := manager.Attach(&fakeTransport{}, Info{
		Name:         "modern",
		OS:           "android",
		Capabilities: []string{" files.read ", "core.exec", "files.read"},
	})
	if !ok {
		t.Fatal("modern attach failed")
	}
	defer manager.Detach(modern)

	listed := manager.List()
	if len(listed) != 1 {
		t.Fatalf("device count=%d, want 1", len(listed))
	}
	want := []string{"core.exec", "files.read"}
	if fmt.Sprint(listed[0].Capabilities) != fmt.Sprint(want) {
		t.Fatalf("capabilities=%v, want %v", listed[0].Capabilities, want)
	}
	if err := manager.RequireCapability("modern", "core.exec"); err != nil {
		t.Fatalf("core.exec rejected: %v", err)
	}
	if err := manager.RequireCapability("modern", "core.jobs"); !errors.Is(err, ErrCapabilityUnsupported) {
		t.Fatalf("core.jobs error=%v, want ErrCapabilityUnsupported", err)
	}

	manager.Detach(modern)
	legacy, ok := manager.Attach(&fakeTransport{}, Info{Name: "legacy", OS: "linux"})
	if !ok {
		t.Fatal("legacy attach failed")
	}
	defer manager.Detach(legacy)
	if err := manager.RequireCapability("legacy", "core.jobs"); err != nil {
		t.Fatalf("legacy core.jobs should remain compatible: %v", err)
	}
	if err := manager.RequireCapability("legacy", "device.info"); !errors.Is(err, ErrCapabilityUnsupported) {
		t.Fatalf("legacy new capability error=%v, want ErrCapabilityUnsupported", err)
	}
}
